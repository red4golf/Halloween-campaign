...
(function () {
  ...
  // True only when THIS pageload's URL itself carries ?leader=... (i.e. the
  // visitor just landed off a tracked link), not merely when a code is
  // sitting in sessionStorage from an earlier page in the visit.
  function isFreshLeaderLink() {
    try {
      return !!new URLSearchParams(window.location.search).get('leader');
    } catch (e) {
      return false;
    }
  }

  // Gives a referred visitor visible proof their click did something —
  // otherwise the leader link is a silent no-op, since registration isn't
  // open yet and there's nothing on the page that reacts to ?leader=.
  function showLeaderBanner() {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var freshLeaderLink = isFreshLeaderLink();
    captureLeaderCode(); // capture/persist ?leader= even on pages with no form
    if (freshLeaderLink && document.body) { showLeaderBanner(); }
